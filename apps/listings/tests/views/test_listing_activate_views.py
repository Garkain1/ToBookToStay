...
class TestListingActivateView(APITestCase):
    ...
    def setUp(self):
        ...
        self.listing = Listing.objects.create(
            ...
            status=ListingStatusChoices.DEACTIVATED, owner=self.business_user)

    def test_activate_listing_unauthenticated_user(self):
        # Попытка активации неаутентифицированным пользователем
        url = reverse('listing-activate', kwargs={'id': self.listing.id})
        response = self.client.patch(url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_activate_listing_regular_user(self):
    ...
    def test_activate_listing_business_user(self):
        ...
        self.assertEqual(self.listing.status, ListingStatusChoices.ACTIVE)

    def test_activate_other_user_listing_as_business(self):
    ...
    def test_activate_listing_admin_user(self):
    ...
    def test_activate_deleted_listing(self):
    ...
    def test_activate_deleted_listing_admin_user(self):
        # Попытка активации мягко удаленного объявления админом
        self.listing.status = ListingStatusChoices.DELETED
        self.listing.save()
        self.client.login(email='admin@example.com', password='password')
        url = reverse('listing-activate', kwargs={'id': self.listing.id})
    ...
    def test_invalid_activation_action(self):
        # Отправка некорректного параметра действия
        self.client.login(email='business@example.com', password='password')
        url = reverse('listing-activate', kwargs={'id': 9999})
        response = self.client.patch(url)

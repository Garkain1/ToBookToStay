...
class TestListingSoftDeleteView(APITestCase):
    ...
    def test_soft_delete_listing_unauthenticated_user(self):
        # Попытка активации неаутентифицированным пользователем
        url = reverse('listing-soft-delete', kwargs={'id': self.listing.id})
        response = self.client.patch(url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_soft_delete_listing_regular_user(self):
    ...
    def test_soft_delete_listing_business_user(self):
    ...
    def test_soft_delete_other_user_listing_as_business(self):
    ...
    def test_soft_delete_listing_admin_user(self):
    ...
    def test_invalid_soft_deletion_action(self):
        # Отправка некорректного параметра действия
        self.client.login(email='business@example.com', password='password')
        url = reverse('listing-soft-delete', kwargs={'id': 9999})
        response = self.client.patch(url)

...
class TestListingCreateSerializer(TestCase):
    ...
    def test_create_listing(self):
        serializer = ListingCreateSerializer(data=self.valid_data)
        serializer.is_valid(raise_exception=True)
        listing = serializer.save(owner=self.user)  # Сохраняем новый объект
    ...
    def test_default_values_for_optional_fields(self):
        ...
        optional_data.pop('property_type')  # Убираем необязательное поле
        serializer = ListingCreateSerializer(data=optional_data)
        serializer.is_valid(raise_exception=True)
        listing = serializer.save(owner=self.user)
